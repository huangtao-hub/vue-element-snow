...
public class WxTaskController {
    private final Log logger = LogFactory.getLog(WxTaskController.class);
    ...
    @Autowired
    private LitemallTaskService taskService;
...
//        }
        userId = 7;
        List<LitemallTask> list = taskService.queryByUid(userId);
...
//        result.put("cartList", cartList);
     ...
     *
     * @param userId 用户ID
     * @param task   购物车商品信息， { goodsId: xxx, productId: xxx, number: xxx }
     * @return 返回结果
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody LitemallTask task) {
//        if (userId == null) {
//            return ResponseUtil.unlogin(); //未登录
//        }
//        if (task == null) {
//            return ResponseUtil.badArgument(); //添加为空
//        }
...
//            }
System.out.println(task.getTsskName());
        task.setId(null);
//        task.setSpecifications();
//        task.setPrice();
        task.setTsskName(task.getTsskName());
//        task.setFinishTime();
//        task.setGoodsSn(goods.getGoodsSn());
...
//            }
            task.setTaskSn("进行中");
//        task.setSpecifications(product.getSpecifications());
            task.setPicUrl("https://wpimg.wallstcn.com/f778738c-e4f8-4870-b634-56703b4acafe.gif");
...
//        }
     ...
     *
     * @param userId 用户ID
     * @param task   购物车商品信息， { id: xxx, goodsId: xxx, productId: xxx, number: xxx }
     * @return 修改结果
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody LitemallTask task) {
//        if (userId == null) {
//            return ResponseUtil.unlogin();
//        }
//        Integer productId = cart.getProductId();
...
//        }
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        LitemallTask existtask = taskService.findById(7, 6);
...
//        }
        ...
        existtask.setSpecifications("9999");
        if (taskService.updateById(existtask) == 0) {
            return ResponseUtil.updatedDataFailed();
        }
